...
public class PEKSmain {
  ...
  public C Enc(PK pk, String w) throws UnsupportedEncodingException, NoSuchAlgorithmException {
    ...
    //生成t
    Element h1w = pk.pairing.getG1().newElement()
        .setFromHash(PEKS_Utils.MD5HASH(w), 0, PEKS_Utils.MD5HASH(w).length);
    Element hr = pk.h.duplicate().powZn(r);
    Element t = pk.pairing.pairing(h1w, hr).getImmutable();
    //生成C
    Element c1 = pk.g.duplicate().powZn(r);
    Element c2 = pk.pairing.getG2().newElement()
        .setFromHash(PEKS_Utils.SHA256(t.toString()), 0, PEKS_Utils.SHA256(t.toString()).length);
   ...
   *   Output: td
   * */
  public TD TdGen(PK pk, SK sk, String w)
  ...
  public boolean Test(PK pk, TD td, C c) {
    ...
    Element flag = pk.pairing.getG2().newElement()
        .setFromHash(PEKS_Utils.SHA256(tdc1.toString()), 0,
            PEKS_Utils.SHA256(tdc1.toString()).length);
    if (flag.equals(c.c2)) {
      return true;
    } else {
      return false;
